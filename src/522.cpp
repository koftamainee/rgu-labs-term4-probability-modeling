...
int main() {
    ...
    auto results = runner.run([](std::mt19937& rng) -> SimResult {
        ...
        for (auto v : X)
            if (Y.count(v)) { intersect = true; break; }
        ...
        for (auto c : ciphertexts_1)
            if (ciphertexts_2.count(c)) { collision = true; break; }
    ...
    double p_lower_bound = 1.0 - std::exp(-static_cast<double>(K * K) / N_UNIVERSE);
    ...
    std::println("=== Birthday Problem / Vigenere Attack (N={}, k={}, sims={}) ===", N_UNIVERSE, K, SIM);
    std::println("");
    ...
    std::println("  Current k={} is {} the threshold", K, K > static_cast<size_t>(k_threshold) ? "ABOVE" : "BELOW");
    std::println("");
