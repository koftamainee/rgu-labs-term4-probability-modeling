    ...
    bool                            use_vigenere)
{
    ...
    double pm  = 0.0;
    for (size_t j = 0; j < msgs.size(); ++j)
        if (msgs[j] == target_msg) { pm = msg_probs[j]; break; }
    ...
    double pcm = (p_cipher_given_msg.count(target_msg) &&
                  p_cipher_given_msg.at(target_msg).count(target_cipher))
                     ? p_cipher_given_msg.at(target_msg).at(target_cipher) : 0.0;
    double pmc = (pc > 0.0) ? (pcm * pm / pc) : 0.0;

    std::println("  P(Cipher='{}')             = {:.4f}", target_cipher, pc);
    std::println("  P(Cipher='{}'|M='{}') = {:.4f}", target_cipher, target_msg, pcm);
    std::println("  P(M='{}'|Cipher='{}') = {:.4f}", target_msg, target_cipher, pmc);
    std::println("  Perfect secrecy?           = {}", std::abs(pmc - pm) < 1e-9 ? "YES" : "NO");
}
