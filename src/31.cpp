...
public:
    ClusterSimulator(int clusters_count, int cells_per_cluster, int pattern_length,
        ...
        : m_clusters_count(clusters_count), m_cells_per_cluster(cells_per_cluster),
          m_pattern_length(pattern_length), m_pattern(std::move(pattern)), m_alphabet(std::move(alphabet)),
    ...
    bool are_connected(const std::string& left_cluster, const std::string& right_cluster) const {
        for (int left_length = 1; left_length <= m_cells_per_cluster && left_length < m_pattern_length; ++left_length) {
            for (int right_length = 1; right_length <= m_cells_per_cluster && right_length < m_pattern_length; ++right_length) {
                if (left_length + right_length != m_pattern_length) continue;

                std::string left_part = left_cluster.substr(m_cells_per_cluster - left_length, left_length);
    ...
    void simulate_empirical(int experiments_count) {
        ...
        for (int exp = 0; exp < experiments_count; ++exp) {
            ...
            if (exp == 0) {
                ...
                for (const int idx : connected) {
                    std::cout << "Pairs " << idx+1 << " and " << idx+2 << " are connected\n";
        ...
        std::cout << "\n========== SIMULATION RESULTS ==========\n";
        std::cout << "Parameters: M=" << m_clusters_count << ", n=" << m_cells_per_cluster
        ...
        const double prob_all_connected = static_cast<double>(all_connected_count) / experiments_count;
        std::cout << "a. Probability of all clusters being connected: "
                  << prob_all_connected << " (" << all_connected_count << "/" << experiments_count << ")\n";
        ...
        const double prob_all_disconnected = static_cast<double>(all_disconnected_count) / experiments_count;
        std::cout << "\nc. Probability of all clusters being disconnected: "
                  << prob_all_disconnected << " (" << all_disconnected_count << "/" << experiments_count << ")\n";
    }
...
int main() {
    try {
        ...
        ClusterSimulator simulator(clusters_count, cells_per_cluster, pattern_length,
        ...
        ClusterSimulator simulator2(clusters_count, cells_per_cluster, pattern_length,
